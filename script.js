const translations = {
  es: {
    meta: {
      title: 'Alejandro & Ayane | Información de boda para invitados',
      description:
        'Sitio web de información de boda con cronograma, sedes, detalles de viaje e instrucciones de RSVP para invitados.',
    },
    skip: 'Saltar al contenido principal',
    language: {
      label: 'Idioma',
      selectAria: 'Seleccionar idioma',
      spanish: 'Español',
      japanese: '日本語',
    },
    nav: {
      ariaLabel: 'Navegación principal',
      menuButton: 'Menú',
      home: 'Inicio',
      details: 'Detalles',
      venues: 'Sedes',
      schedule: 'Cronograma',
      rsvp: 'RSVP',
      travel: 'Viaje',
      faq: 'Preguntas',
      gallery: 'Galería',
    },
    countdown: {
      ariaLabel: 'Cuenta regresiva para la boda',
      text: 'días para la boda (1 de octubre de 2026 a las 5:00 PM, hora de Ciudad de México).',
    },
    hero: {
      ariaLabel: 'Introducción de la boda',
      eyebrow: 'Junto con nuestras familias',
      title: 'Alejandro y Ayane se casan',
      date: 'Sábado, 20 de junio de 2026',
      message:
        'Estamos muy agradecidos de celebrar este día tan especial con familia y amigos. Abajo encontrarás todos los detalles importantes en un solo lugar.',
      cta: 'Ver detalles',
    },
    details: {
      title: 'Detalles importantes',
      dateTitle: 'Fecha',
      dateValue: 'Sábado, 20 de junio de 2026',
      timeTitle: 'Hora',
      timeValue: '3:00 PM Ceremonia | 5:00 PM Recepción',
      venueTitle: 'Lugar principal',
      venueValue: 'Sunset Garden Estate',
      dressTitle: 'Código de vestimenta',
      dressValue: 'Formal / Semiformal (se recomiendan colores claros)',
      contactTitle: 'Contacto',
      contactValue: 'Jordan (coordinación de boda): +1 (000) 000-0000',
    },
    venues: {
      title: 'Sedes',
      ceremonyTitle: 'Sede de la ceremonia',
      ceremonyName: 'Starlight Chapel',
      ceremonyAddress: '123 Blossom Lane, Your City, ST 00000',
      mapsLink: 'Marcador de Google Maps',
      ceremonyNote: 'Por favor llega 20 minutos antes para tomar asiento.',
      receptionTitle: 'Sede de la recepción',
      receptionName: 'Sunset Garden Estate',
      receptionAddress: '456 Golden Road, Your City, ST 00000',
      receptionNote: 'Habrá personal de estacionamiento para guiar a los invitados.',
    },
    schedule: {
      title: 'Cronograma del día de la boda',
      arrival: 'Llegada de invitados',
      ceremony: 'Inicio de la ceremonia',
      photos: 'Fotos de familia y pareja',
      dinner: 'Servicio de cena',
      dancing: 'Baile y celebración',
      end: 'Fin del evento',
    },
    rsvp: {
      title: 'Lista de invitados y RSVP',
      line1Html:
        'Confirma tu asistencia antes del <strong>15 de mayo de 2026</strong> para finalizar asientos y catering.',
      line2: 'Si recibiste una invitación, responde usando el formulario de RSVP de abajo.',
      button: 'Marcador de formulario RSVP',
      buttonAria: 'Abrir formulario de RSVP',
    },
    travel: {
      title: 'Viaje y alojamiento',
      hotelsTitle: 'Recomendaciones de hotel',
      hotelA: 'Hotel A (a 10 minutos del lugar)',
      hotelB: 'Hotel B (centro de la ciudad, con transporte)',
      hotelC: 'Hotel C (opción económica cercana)',
      transportTitle: 'Información de transporte',
      transport1: 'Hay zona de descenso para apps de transporte en ambas sedes.',
      transport2: 'Servicio de transporte limitado hacia la recepción.',
      transport3: 'También hay opciones de taxi y transporte público local.',
      airportTitle: 'Notas de aeropuerto / ciudad',
      airport1: 'Aeropuerto más cercano: Your City International Airport (YYY).',
      airport2: 'Aprox. 35 minutos al lugar principal en auto.',
      airport3: 'Para invitados del extranjero: recuerden seguro de viaje y adaptadores.',
    },
    faq: {
      title: 'Preguntas frecuentes',
      q1: '¿Puedo llevar acompañante?',
      a1: 'Revisa tu invitación. Si tienes dudas, contáctanos directamente.',
      q2: '¿Están invitados los niños?',
      a2: '¡Queremos mucho a tus peques! Revisa tu invitación para confirmar asistencia infantil.',
      q3: '¿Qué debo usar?',
      a3: 'Ropa formal o semiformal, cómoda y lista para celebrar.',
      q4: '¿Dónde debo estacionarme?',
      a4: 'Habrá estacionamiento en sitio con personal en la sede de recepción.',
      q5: '¿A qué hora debo llegar?',
      a5: 'Por favor llega entre 20 y 30 minutos antes de que inicie la ceremonia.',
      q6: '¿A quién contacto si tengo dudas?',
      a6: 'Contacta a Jordan al +1 (000) 000-0000 para ayuda.',
    },
    gallery: {
      title: 'Galería de fotos',
      introHtml:
        'Reemplaza estas imágenes de ejemplo con tus propias fotos en <code>assets/images</code>.',
      ariaLabel: 'Marcadores de posición de galería',
      alt1: 'Imagen de ejemplo para foto de compromiso 1',
      alt2: 'Imagen de ejemplo para foto de compromiso 2',
      alt3: 'Imagen de ejemplo para foto de compromiso 3',
      alt4: 'Imagen de ejemplo para foto de compromiso 4',
      alt5: 'Imagen de ejemplo para foto de compromiso 5',
      alt6: 'Imagen de ejemplo para foto de compromiso 6',
    },
    footer: {
      thanks: 'Gracias por celebrar con nosotros.',
      date: '20 de junio de 2026',
      top: 'Volver arriba',
    },
  },
  ja: {
    meta: {
      title: 'Alejandro & Ayane | 結婚式ゲスト向け情報',
      description:
        '結婚式のスケジュール、会場、旅行情報、RSVP案内をまとめたゲスト向けサイトです。',
    },
    skip: 'メインコンテンツへ移動',
    language: {
      label: '言語',
      selectAria: '言語を選択',
      spanish: 'Español',
      japanese: '日本語',
    },
    nav: {
      ariaLabel: 'メインナビゲーション',
      menuButton: 'メニュー',
      home: 'ホーム',
      details: '詳細',
      venues: '会場',
      schedule: 'スケジュール',
      rsvp: 'RSVP',
      travel: '旅行情報',
      faq: 'よくある質問',
      gallery: 'ギャラリー',
    },
    countdown: {
      ariaLabel: '結婚式までのカウントダウン',
      text: '日後に結婚式です（2026年10月1日 午後5:00 メキシコシティ時間）。',
    },
    hero: {
      ariaLabel: '結婚式のご案内',
      eyebrow: '両家の家族とともに',
      title: 'Alejandro と Ayane が結婚します',
      date: '2026年6月20日（土）',
      message:
        '家族や友人の皆さまとこの特別な日を迎えられることをとても嬉しく思っています。必要な情報をこのページにまとめました。',
      cta: '詳細を見る',
    },
    details: {
      title: '重要なお知らせ',
      dateTitle: '日付',
      dateValue: '2026年6月20日（土）',
      timeTitle: '時間',
      timeValue: '午後3:00 挙式 | 午後5:00 披露宴',
      venueTitle: 'メイン会場',
      venueValue: 'Sunset Garden Estate',
      dressTitle: 'ドレスコード',
      dressValue: 'フォーマル / セミフォーマル（明るい色がおすすめ）',
      contactTitle: '連絡先',
      contactValue: 'Jordan（ウェディングコーディネーター）：+1 (000) 000-0000',
    },
    venues: {
      title: '会場',
      ceremonyTitle: '挙式会場',
      ceremonyName: 'Starlight Chapel',
      ceremonyAddress: '123 Blossom Lane, Your City, ST 00000',
      mapsLink: 'Googleマップ（仮リンク）',
      ceremonyNote: '着席のため、20分前までにお越しください。',
      receptionTitle: '披露宴会場',
      receptionName: 'Sunset Garden Estate',
      receptionAddress: '456 Golden Road, Your City, ST 00000',
      receptionNote: '到着時に駐車場スタッフがご案内します。',
    },
    schedule: {
      title: '結婚式当日のスケジュール',
      arrival: 'ゲスト受付開始',
      ceremony: '挙式開始',
      photos: '家族・新郎新婦の写真撮影',
      dinner: 'ディナー',
      dancing: 'ダンス＆お祝い',
      end: '閉会',
    },
    rsvp: {
      title: 'ゲストリスト＆RSVP',
      line1Html:
        '<strong>2026年5月15日</strong> までに出欠をご回答ください。席次とケータリングの準備に必要です。',
      line2: '招待状を受け取った方は、以下のRSVPフォームからご回答ください。',
      button: 'RSVPフォーム（仮リンク）',
      buttonAria: 'RSVPフォームを開く',
    },
    travel: {
      title: '交通・宿泊案内',
      hotelsTitle: 'ホテルのおすすめ',
      hotelA: 'Hotel A（会場から10分）',
      hotelB: 'Hotel B（市内中心部、シャトルあり）',
      hotelC: 'Hotel C（近隣の手頃なホテル）',
      transportTitle: '交通情報',
      transport1: '両会場とも配車サービスの降車エリアがあります。',
      transport2: '披露宴会場行きのシャトルは一部のみ運行予定です。',
      transport3: 'タクシーと公共交通機関も利用できます。',
      airportTitle: '空港 / 都市メモ',
      airport1: '最寄り空港：Your City International Airport (YYY)',
      airport2: 'メイン会場までは車で約35分です。',
      airport3: '海外からのゲストは旅行保険と変換プラグをご準備ください。',
    },
    faq: {
      title: 'よくある質問',
      q1: '同伴者を連れて行けますか？',
      a1: '招待状をご確認ください。不明な場合は直接ご連絡ください。',
      q2: '子どもの参加は可能ですか？',
      a2: 'お子さまも大歓迎です。招待状の記載内容をご確認ください。',
      q3: '服装は何がよいですか？',
      a3: 'フォーマルまたはセミフォーマルで、動きやすい服装でお願いします。',
      q4: '駐車場はどこですか？',
      a4: '披露宴会場にスタッフ付きの駐車場があります。',
      q5: '何時に到着すればいいですか？',
      a5: '挙式開始の20〜30分前までにお越しください。',
      q6: '質問がある場合の連絡先は？',
      a6: 'ご不明点は Jordan（+1 (000) 000-0000）までご連絡ください。',
    },
    gallery: {
      title: 'フォトギャラリー',
      introHtml:
        '<code>assets/images</code> のサンプル画像を、おふたりの写真に差し替えてください。',
      ariaLabel: 'フォトギャラリーのプレースホルダー',
      alt1: '婚約写真1のプレースホルダー',
      alt2: '婚約写真2のプレースホルダー',
      alt3: '婚約写真3のプレースホルダー',
      alt4: '婚約写真4のプレースホルダー',
      alt5: '婚約写真5のプレースホルダー',
      alt6: '婚約写真6のプレースホルダー',
    },
    footer: {
      thanks: '私たちの大切な日を一緒にお祝いしてくださりありがとうございます。',
      date: '2026年6月20日',
      top: 'ページ上部へ戻る',
    },
  },
};

const navToggle = document.querySelector('.nav-toggle');
const navMenu = document.querySelector('.nav-menu');
const languageToggle = document.querySelector('#language-toggle');
const countdownDays = document.querySelector('#countdown-days');
const countdownLabel = document.querySelector('#countdown-label');

const targetDate = new Date('2026-10-01T17:00:00-06:00');
const millisecondsPerDay = 1000 * 60 * 60 * 24;

let currentLanguage = 'es';

const getTranslation = (lang, key) => {
  return key.split('.').reduce((value, part) => value?.[part], translations[lang]);
};

const applyTranslations = (lang) => {
  currentLanguage = translations[lang] ? lang : 'es';
  document.documentElement.lang = currentLanguage;

  document.querySelectorAll('[data-i18n]').forEach((element) => {
    const key = element.dataset.i18n;
    const translation = getTranslation(currentLanguage, key);
    if (!translation) {
      return;
    }

    if (element.dataset.i18nHtml === 'true') {
      element.innerHTML = translation;
    } else {
      element.textContent = translation;
    }
  });

  document.querySelectorAll('[data-i18n-attr]').forEach((element) => {
    const pairs = element.dataset.i18nAttr.split('|');

    pairs.forEach((pair) => {
      const [attribute, key] = pair.split(':');
      const translation = getTranslation(currentLanguage, key);
      if (attribute && translation) {
        element.setAttribute(attribute.trim(), translation);
      }
    });
  });

  updateCountdown();
};

const updateCountdown = () => {
  if (!countdownDays || !countdownLabel) {
    return;
  }

  const now = new Date();
  const difference = targetDate.getTime() - now.getTime();

  countdownDays.textContent = difference <= 0 ? '0' : String(Math.ceil(difference / millisecondsPerDay));
  countdownLabel.textContent = getTranslation(currentLanguage, 'countdown.text');
};

if (navToggle && navMenu) {
  navToggle.addEventListener('click', () => {
    const isOpen = navMenu.classList.toggle('open');
    navToggle.setAttribute('aria-expanded', String(isOpen));
  });

  navMenu.querySelectorAll('a').forEach((link) => {
    link.addEventListener('click', () => {
      navMenu.classList.remove('open');
      navToggle.setAttribute('aria-expanded', 'false');
    });
  });
}

window.addEventListener('keydown', (event) => {
  if (event.key === 'Escape' && navMenu?.classList.contains('open')) {
    navMenu.classList.remove('open');
    navToggle?.setAttribute('aria-expanded', 'false');
    navToggle?.focus();
  }
});

if (languageToggle) {
  languageToggle.value = 'es';
  languageToggle.addEventListener('change', (event) => {
    applyTranslations(event.target.value);
  });
}

applyTranslations('es');
setInterval(updateCountdown, 60 * 1000);
